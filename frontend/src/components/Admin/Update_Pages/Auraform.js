...
const Auraform = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/getAuraPage");
        setFormField(response.data);
        if (response.data.AuraBanner) {
          setImagePreview(response.data.AuraBanner);
        }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let imageUrl = formField.AuraBanner;
    ...
    try {
      ...
      await axios.put("/api/update/Aura", updatedData);
              ...
              <div className="card card-orange">
                <div className="card-header">
                  <h3 className="card-title">
                    <span className="text-white">Aura Form</span>
                  </h3>
                </div>
