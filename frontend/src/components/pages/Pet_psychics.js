...
useEffect(() => {
  ...
  const fetchProfiles = async () => {
    try {
      const response = await axios.get(
        `/api/profilestep2/category/Pet Psychics?page=${currentPage}&limit=6`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/getpetpage"); // Update the URL if needed
    ...
    <div>
      <div className="container">
        ...
        <div className="love_banner" data-aos="zoom-in-up">
          <img src={crystalPage.PetBanner} alt="love banner" />
        </div>
        <br></br>
        <br></br>
