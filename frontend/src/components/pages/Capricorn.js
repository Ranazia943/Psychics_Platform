...
import Horoscopes_add from "../Images/Horoscopes_add.png"; // This is your image import; ensure the path is correct
...
const Psychicvop = () => {
  ...
  useEffect(() => {
    ...
    const fetchHoroscopesData = async () => {
      try {
        const response = await axios.get("/api/get/capricorn"); // Update the URL if needed
    ...
    const fetchProfiles = async () => {
      try {
        const response = await axios.get(`/api/psychics/data/category/Numerologist`);
    ...
    fetchHoroscopesData(); // Call to fetch horoscope data
    fetchProfiles(); // Call to fetch psychic profiles
