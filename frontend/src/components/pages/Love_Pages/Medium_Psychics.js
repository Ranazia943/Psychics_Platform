  ...
  const fetchProfiles = async () => {
    try {
      const response = await axios.get(
        `/api/profilestep2/category/Parents Children?page=${currentPage}&limit=6`
      );
  ...
  const handlePageChange = (page) => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axios.get("/api/astrology"); // Update the URL if needed
...
export default Medium_Psychics;
