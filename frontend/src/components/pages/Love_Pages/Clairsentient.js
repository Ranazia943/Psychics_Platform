...
const Clairsentient = () => {
  ...
  const fetchProfiles = async () => {
    try {
      const response = await axios.get(
        `/api/profilestep2/category/Clarisentient?page=${currentPage}&limit=6`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/clair_sentient"); // Update the URL if needed
